...
def run_batches(session, batch_generator, input_p_ph, input_l_ph, target_ph, input_p_op, input_l_op, target_op, output_ops, step_op, loss_op):
    # Init counters / stats.
    start_time = time.time()
    solved, count, loss = (0.0, 0.0, 0.0)
    try:
        if INFERENCE_OUT is not None:
            run_batches.of = run_batches.of
    except:
        if INFERENCE_OUT is not None:
            run_batches.of = open(INFERENCE_OUT,"w")        
    ...
    for b, batch in enumerate(batch_generator()):
        ...
        if INFERENCE_OUT is not None:
            out = run_values["outputs"][0]
            for item in out:
                run_batches.of.write("%d %f %f\n"%(np.argmax(item),item[0],item[1]))
            run_batches.of.flush()
    elapsed = time.time() - start_time
    # Return stats.
...
def parse_args():
    ...
    if args.hvd:
        ...
        import dl_comm.tensorflow as cdl_temp
        cdl = cdl_temp
        #hvd.init()
        RANK = cdl.get_rank()
        RANKS = cdl.get_nranks()
    if RANK != 0:
        DEBUG = False
...
def build_optimizer(args,loss_op,num_train_items):
    ...
    if args.use_clr:
        ...
        step_sz = 10*(num_train_items/args.batch_size)
        max_steps = args.epochs*RANKS*(num_train_items/args.batch_size)
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate) #tf.train.GradientDescentOptimizer(learning_rate)
    if args.hvd:
...
def run_gnn(args,model_ops,test_items,train_items=None,optimizer=None):
    ...
    if args.hvd:
        ...
        bcast_op = cdl.broadcast(tf.all_variables,0)
        sess.run(bcast_op)
    # Save / Restore model.
    ...
    for epoch in range(args.epochs):
        ...
        if RANK == 0:
            print("  Testing.")
        elapsed, solved, loss, count = run_batches(sess, lambda: item_batch_iter(test_items,args.batch_size*2, shuffle=False),
                                                   inputs_p_ph, inputs_l_ph, targets_ph,
                                                   inputs_p_op, inputs_l_op, targets_op,
        ...
        if RANK == 0:
            ...
            #plot_history(log_epochs, solveds_tr, solveds_ge, 'MLDock-Accuracy')
            #plot_history(log_epochs, losses_tr, losses_ge, 'MLDock-Loss')
            #plot_history(log_epochs, lr_hist, lr_hist, 'MLDock-LR')
        if INFERENCE_ONLY:
