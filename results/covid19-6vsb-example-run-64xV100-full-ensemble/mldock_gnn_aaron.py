...
#           *---------*  |  *------*  |  *---------*
#           |         |  |  |      |  |  |         |
# Input --->| Encoder |  *->| Core |--*->| Decoder |---> Output(N)
#           |         |---->|      |     |         |
#           *---------*     *------*     *---------*
#
############################################################
...
def item_batch_iter(items, batch_size):
    ...
    data_threads = du.DataLoader(items,batch_size*4,nthreads=2)
    data_threads.start()
...
classifier1 = snt.Linear(256,initializers=initers, regularizers=regs)
final_output = snt.Linear(2, initializers=initers, regularizers=regs)
output_ops = [final_output(classifier1(tf.nn.dropout(classifier0(merged_output),keep_prob=0.5)))]
if MODE=='classification':
