...
#SBATCH -C V100
#SBATCH -p spider
#SBATCH --exclude=spider-0013,spider-0002
#SBATCH --exclusive
##SBATCH -e stderr.out
##SBATCH -o stdout.out
#SBATCH -t 2:00:00
...
#list_of_files= #"bindingdb_2019m4_5of25pct_d
list_of_files="bindingdb_2019m4_5of25pct_e"
model_n=4
...
#-rw-r--r--. 1 avose criemp 149M Jun 23 07:06 /lus/scratch/avose/data/map/bindingdb_2019m4_75.map
    ...
    #salloc -N 1 -C V100 -p spider --exclusive
    #srun  -l -N ${NODES} -n ${NP} -C V100 -p spider -u --exclusive --cpu_bind=none python mldock_gnn.py \
    #horovodrun -np ${NP} python mldock_gnn.py \
    #mpirun --allow-run-as-root --tag-output -np 2 -H spider-0002:2 -bind-to none -map-by slot -mca pml ob1 -mca btl ^openib  -mca btl_tcp_if_include lo python mldock_gnn.py \
    srun -c 9 --hint=multithread -l -N ${NODES} -n ${NP} --ntasks-per-node=8 --ntasks-per-socket=4 -u --exclusive --cpu_bind=none --accel-bind=g,v python mldock_gnn.py \
