...
def build_optimizer(args,loss_op,num_train_items):
    ...
    if args.use_clr:
        ...
        lr_decay = clr.cyclic_learning_rate(global_step=global_step, learning_rate=lr_init*.001, max_lr=lr_init*1000,
                                                 step_size=50*step_sz, mode='exp_range', gamma=.999) #tf.train.exponential_decay(lr_init, global_step, max_steps*10, 0.9, staircase=False)
        learning_rate = clr.cyclic_learning_rate(global_step=global_step, learning_rate=lr_decay*0.001, max_lr=2*lr_decay,
                                                 step_size=step_sz, mode='triangle', gamma=.999)
