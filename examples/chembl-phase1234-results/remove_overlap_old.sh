...
cat ensemble5x_covid19_results_*_chembl_phase1234/top-compounds-*-ID-only-ranked-fp.out | wc -l
...
do
    ...
    num_ol=0
    num_uniq=0
    total_drugs=$(( `cat ensemble5x_covid19_results_${PDB_ID}_chembl_phase1234/top-compounds-${PDB_ID}-ID-only-ranked-fp.out | wc -l` ))
    total_overlap=$(( `cat overlap_only.txt | wc -l` ))

    cat overlap_only.txt ensemble5x_covid19_results_${PDB_ID}_chembl_phase1234/top-compounds-${PDB_ID}-ID-only-ranked-fp.out | sort > ${PDB_ID}_sorted.txt
    cat ${PDB_ID}_sorted.txt | uniq -u > ${PDB_ID}_uniq_only.txt
    total_uniq=$(( `cat ${PDB_ID}_uniq_only.txt | wc -l` ))
    

    for CHEMBL in $(cat < ensemble5x_covid19_results_${PDB_ID}_chembl_phase1234/top-compounds-${PDB_ID}-ID-only-ranked-fp.out); do
        #echo "CHEMBL=" $CHEMBL
    ...
    echo "<html><body>PharML Top Ranked Compounds" > ${HTML_OUTPUT}
